...
setInterval(() => {
  if(gamepadIndex !== undefined) {
    ...
    document.body.innerHTML = " ESP-Rider Going Wild!"; // reset page
    ...
    myGamepad.buttons.map(e => e.pressed).forEach((isPressed, buttonIndex) => { 
      if(isPressed) {
        if(buttonIndex == 0){ toggle("D");}   // sidle left
        if(buttonIndex == 1){ toggle("G");}   // forward 
        if(buttonIndex == 2){ toggle("B");}   // back
        if(buttonIndex == 3){ toggle("E");}   // sidle right  diagonals not done yet
        if(buttonIndex == 4){ toggle("off");}

        // button is pressed; indicate this on the page so when more buttons need to be programmed uncomment the next line
